...
X_train,X_test,y_train,y_test=train_test_split(x_data, y, test_size=0.99)
...
cnn.load_state_dict(torch.load('./model-baseline6.pkl'))
cnnnext=next(2)
...
def evaluate(cnnnext, criteon):
    avg_acc = []
    cnnnext.eval()         #表示进入测试模式
...
for epoch in range(15):
    ...
    test_acc = evaluate(cnnnext, criteon)
